...
int generateattack(int min, int max) {
    srand((unsigned int)time(NULL));
    return 50 + rand() % (51 - 25);// genera el ataque entre 51 y 25
}
...
int main()
{
    ...
    while (heroIsAlive && (enemyIsAlive1 || enemyIsAlive2)) {
        ...
        if (enemyChoosed == 1) {
            ...
            enemyHP1 = heroAttackEnemy(enemyName1, enemyHP1,enemydamage);
            enemyIsAlive1 = checkEnemyStatus(enemyName1, enemyHP1);
        }
        else {
            //ATACAMOS AL ENEMIGO 2
            enemyHP2 = heroAttackEnemy(enemyName2, enemyHP2,enemydamage );
        ...
        if (enemyIsAlive1 && heroIsAlive) {
            enemyAttack(enemyName1, heroHP,enemydamage);
        }
        //ATACA EL ENEMIGO 2 SI ESTÁ VIVO Y EL HÉROE TAMBIÉN
        if (enemyIsAlive2 && heroIsAlive) {
            enemyAttack(enemyName2, heroHP, enemydamage);
        } 
    }
}
